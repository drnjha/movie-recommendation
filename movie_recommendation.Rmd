...
The final result demonstrates that by including several predictive markers in a simple model, a considerable gain in prediction accuracy can be achieved. 
...
$$
\frac{1}{N}\sum_{u,i,g,a}(y_{u,i,g,a} - \mu - b_i - b_u - b_g - b_a)^2 + \lambda\left(\sum_i b_i^2 + \sum_u b_u^2 + \sum_g b_g^2 + \sum_a b_a^2\right)
$$

In the present case a $\lambda$ was picked by cross-validation using a sequence of possible $\lambda$s from `0` to `10` with increasing intervals from `0.05` to `0.25`. From the following graph (Fig. \@ref(fig:plotLambdasVsRMSEs)) it can be seen, however, that using regularization in the context of the present model does *not* contribute to a better prediction. So the final results will be computed without the use of regularization.
...
```
The final result of using *movie*, *user*, combination of *genres* for movies, and *age of movie* at the time of rating results in the RMSE = **`r formatC(RMSE(test_mov_usr_gen_age_pred, validation$rating), format="f", digits=5)`**, which in the context of the present task seems acceptable.

\newpage

# Concluding Remarks

While the first estimate using only the overall mean resulted in an RSME larger than 1.0, the final model resulted in a RMSE that was improved approx. 18.5 %. The regularization in the present model did not add to the accuracy of prediction and thus was not used in the calculation of fonal results. 
